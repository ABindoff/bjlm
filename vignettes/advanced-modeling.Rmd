---
title: "Advanced Modeling with bjlm"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Advanced Modeling with bjlm}
...
While `bjlm` is efficient for standard piecewise regression, its real power lies in its ability to handle structural changes that are themselves functions of other variables. This vignette demonstrates "edge cases" where `bjlm` offers flexibility that is difficult to achieve with other packages.
...
ggplot(dat_market, aes(x = month, y = price, color = ticker)) +
  ...
  geom_line(aes(y = y_fit), linewidth = 1) +
  geom_point(
...
cat(sprintf("Prob NVDA led MSFT: %.2f\n", p1_nvda_msft))
cat(sprintf("Prob NVDA led AAPL: %.2f\n", p1_nvda_aapl))

cat("\n--- Event 2: Autumn Shift (BP 4) ---\n")
cat(sprintf("Prob NVDA led MSFT: %.2f\n", p4_nvda_msft))
cat(sprintf("Prob MSFT led AAPL: %.2f\n", p4_msft_aapl))

cat("\n--- Cross-Event Dynamics ---\n")
cat(sprintf("Prob NVDA AI Surge (BP 2) led AAPL Recovery (BP 1): %.2f\n", p_cross))
```
